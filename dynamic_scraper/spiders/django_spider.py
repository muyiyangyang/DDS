...
class DjangoSpider(DjangoBaseSpider):
	...
	#self.loader.add_xpath('description', '//p[not(@align="center")]/text()')
	#self.loader.add_xpath('//p/a/text()', TakeFirst(), re='the news_content is (.*)')
	item = self.loader.load_item()
        for key, value in item.items():
            if value == None and \
               ...
               not self.scraped_obj_class()._meta.get_field(key).null:
                item[key] = ''
	    if key == 'description':
		aa = self.loader.get_xpath('//p[not(@align="center")]/text()')
		bb = ''.join(i for i in aa)
		self.loader.replace_value('description',bb)
		self.loader.load_item()
		print 'descriptiondescriptiondescriptiondescriptiondescriptiondescriptiondescription'
        	#cc = ''.join(row[0])
		seg_list = jieba.cut(bb,cut_all=False)
		cc = "/".join(seg_list)
		print "Default Mode:", cc
		ab = MySQLdb.connect("localhost","root","wyf1993","online",charset='utf8') 
        	cursor = ab.cursor()
		cursor.execute("INSERT INTO open_news_article1(description) VALUES (%s)",cc)
